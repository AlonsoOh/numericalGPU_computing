...
short vert[4] = { -1, 0, 1, 0 };
short hori[4] = { 0, -1, 0, 1 };
    ...
    for (int iter = 0; iter < we->Jacobian_iterations; iter++) { // Jacobian iterations
        for (int j = 1; j < n_nodes_1d - 1; j++) {
            for (int i = 1; i < n_nodes_1d - 1; i++) {
                ...
                float sum = 0;
                for (int k = 0; k < 4; k++) {
                    int ni = i + vert[k], nj = j + hori[k];
                    sum += grid_next[Y(n_nodes_1d * ni + nj)];
                }
                float b = 2 * grid_cur[Y(n_nodes_1d * i + j)] - grid_prev[Y(n_nodes_1d * i + j)];
                grid_next[Y(n_nodes_1d * i + j)] = (b + we->beta * sum) / diagonal;           
...
int initialize_OpenCL(CONTEXT_WE* cont) {
    ...
    ocl_stuffs->prog_src.length = read_kernel_from_file("Kernel/wave_equation.cl", &ocl_stuffs->prog_src.string);
